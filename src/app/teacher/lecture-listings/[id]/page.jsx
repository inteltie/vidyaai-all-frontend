...
const LecturePage = ({ params }) => {
  ...
  const videoPlayer = useMemo(() => <VideoPlayer id={id} duration={lectureData?.duration} lang={lang}/>, [id,lectureData?.duration, lang]);
  ...
  const lectureOverview = useMemo(
    () => <LectureOverview lectureId={id} isEdit={true} lang={lang} />,
    [id, lang]
  );
  const lectureDetails = useMemo(
    () => <LectureDetails id={id} classID={classID} lang={lang} />,
    [id, classID, lang]
  );
  const articles = useMemo(() => <Articles lectureId={id} />, [id]);
  const lectureAnalytics = useMemo(
    () => <LectureAnalytics lectureId={id} lang={lang}/>,
    [id, lang]
  );
