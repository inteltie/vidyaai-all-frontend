...
import HindiVirtualKeyboard from "@/commonComponents/HindiVirtualKeyboard/HindiVirtualKeyboard";
...
const CommentCard = ({
  ...
  const handleKeyboardChange = (input) => {
    if (input !== text) {
      setText(input); // Only update if input is different
    }
  };
      ...
      <Box mt="auto" pt={2}>
        ...
        <Box display={"flex"} justifyContent={"space-around"}>
          ...
          >
            ...
            value={text}
            onChange={(event) => setText(event.target.value)}
            onFocus={() => setShowKeyboard(true)} // Show keyboard on focus
            onBlur={() => setShowKeyboard(false)} // Hide keyboard on blur
          /> */}
            <IconButton
              color="primary"
              sx={{
